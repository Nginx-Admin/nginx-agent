...
> 当前版本：**v0.5.0**（删除子配置需与 nginx-admin **v0.13.0+** 配套使用）。
...
| 配置删除 | 删除子配置文件（默认禁止删主配置）；走与写入相同的安全闭环 |
| 安全闭环 | 变更前快照 → 写入/删除 → `nginx -t` → reload；任一步失败自动回滚 |
...
删除前快照
    ↓
删除文件（主配置受 allow_main_config 约束，默认拒绝）
    ↓
nginx -t ──失败──► 从快照恢复文件
    ↓ 通过
reload ──失败──► 从快照恢复 + 再次 reload 尽量恢复服务
    ↓ 通过
返回 ok
```
